export const blogPosts = [
  {
    id: 1,
    image:
      "https://digital.library.tu.ac.th/tu_dc/digital/file_upload/biblio/cover/178780.jpg",
    category: "Self-improvement",
    title: "Atomic Habits: Small Changes, Remarkable Results",
    description:
      "Discover how tiny daily habits can lead to extraordinary personal and professional growth over time.",
    author: "Pataveekorn C.",
    date: "07 August 2025",
    likes: 321,
    content: 
      "## 1. Why Habits Matter\n\nJames Clear explains how small, consistent actions compound into remarkable results.\n\n## 2. The Four Laws of Behavior Change\n\nLearn the framework: make it obvious, make it attractive, make it easy, and make it satisfying.\n\n## 3. Breaking Bad Habits\n\nStrategies for identifying triggers and replacing harmful routines.\n\n## 4. Habit Stacking\n\nA method to link new habits to existing ones for easier adoption.\n\n## 5. Long-term Mindset\n\nShifting your focus from goals to systems for sustainable success."
  },
  {
    id: 2,
    image:
      "https://growthsummary.com/wp-content/uploads/2024/06/107.png",
    category: "Business & Finance",
    title: "Rich Dad Poor Dad: Lessons on Wealth and Mindset",
    description:
      "A timeless guide to financial education, changing the way you think about money, work, and success.",
    author: "Pataveekorn C.",
    date: "07 August 2025",
    likes: 278,
    content:
      "## 1. Two Fathers, Two Mindsets\n\nRobert Kiyosaki shares contrasting lessons from his 'rich dad' and 'poor dad'.\n\n## 2. The Importance of Financial Education\n\nWhy schools don’t teach it and how to learn it yourself.\n\n## 3. Assets vs. Liabilities\n\nThe core concept that separates the wealthy from the struggling.\n\n## 4. Entrepreneurship and Investment\n\nBuilding income streams beyond a paycheck.\n\n## 5. Taking Control of Your Future\n\nDeveloping the mindset and skills to achieve financial independence."
  },
  {
    id: 3,
    image:
      "https://online.anyflip.com/bvuvy/fwmz/files/large/7c68645d71b803bf0ba2f22519f73e08.jpg?1678977824",
    category: "Leadership",
    title: "Leaders Eat Last: Creating Teams That Thrive",
    description:
      "Simon Sinek explores why great leaders prioritize their teams and build trust to inspire success.",
    author: "Pataveekorn C.",
    date: "07 August 2025",
    likes: 198,
    content:
      "## 1. The Circle of Safety\n\nHow leaders create environments where people feel secure and valued.\n\n## 2. Trust as the Foundation\n\nThe role of trust in fostering collaboration and innovation.\n\n## 3. Putting People First\n\nWhy leaders who serve their teams see better results.\n\n## 4. The Science of Leadership\n\nBiological and psychological factors that influence leadership.\n\n## 5. Sustaining Strong Cultures\n\nMaintaining morale and unity in times of challenge."
  },
  {
    id: 4,
    image:
      "https://image.makewebcdn.com/makeweb/m_1920x0/Z9S9L5BrM/DefaultData/Deep_Work_final_20_9_22_02.jpg",
    category: "Productivity",
    title: "Deep Work: Rules for Focused Success",
    description:
      "Cal Newport reveals how to master deep work to achieve more in less time and produce higher-quality results.",
    author: "Pataveekorn C.",
    date: "07 August 2025",
    likes: 245,
    content:
      "## 1. What is Deep Work?\n\nThe ability to focus without distraction on cognitively demanding tasks.\n\n## 2. Benefits of Deep Work\n\nImproved output, faster learning, and career advancement.\n\n## 3. Rules for Deep Work\n\nWork deeply, embrace boredom, quit social media, and drain the shallows.\n\n## 4. Building a Deep Work Habit\n\nScheduling focus blocks and eliminating distractions.\n\n## 5. Measuring Progress\n\nTracking results and refining your deep work practice."
  },
  {
    id: 5,
    image:
      "https://imgv2-2-f.scribdassets.com/img/document/563609118/original/c153429fdc/1?v=1",
    category: "Self-improvement",
    title: "Ikigai: The Japanese Secret to a Long and Happy Life",
    description:
      "Discover the ancient Japanese concept of Ikigai and how it can bring purpose and joy to your everyday life.",
    author: "Pataveekorn C.",
    date: "07 August 2025",
    likes: 310,
    content:
      "## 1. Defining Ikigai\n\nFinding the intersection between what you love, what you’re good at, what the world needs, and what you can be paid for.\n\n## 2. The Okinawan Lifestyle\n\nLessons from one of the world’s longest-living communities.\n\n## 3. Flow and Purpose\n\nHow to enter flow states and connect them to your life's mission.\n\n## 4. Healthy Habits\n\nDaily practices that support longevity and happiness.\n\n## 5. Bringing Ikigai to Life\n\nPractical steps to integrate Ikigai into your personal and professional world."
  },
  {
    id: 6,
    image:
      "https://m.media-amazon.com/images/I/71sxTeZIi6L.jpg",
    category: "Business & Finance",
    title: "The Lean Startup: Building a Business for the Future",
    description:
      "Eric Ries shares a scientific approach to creating and managing successful startups in an age of uncertainty.",
    author: "Pataveekorn C.",
    date: "07 August 2025",
    likes: 289,
    content:
      "## 1. The Lean Startup Methodology\n\nBuild, measure, learn — the iterative cycle for innovation.\n\n## 2. Validated Learning\n\nTesting ideas with real customers before heavy investment.\n\n## 3. Minimum Viable Product (MVP)\n\nLaunching small to learn fast and improve.\n\n## 4. Pivot or Persevere\n\nDeciding whether to change direction or continue based on data.\n\n## 5. Continuous Innovation\n\nCreating a culture that embraces experimentation and adaptability."
  }
];
